...
int main(int argc, char *argv[])
{
    ...
    // AFL searches for the shared memory ID literal to check for instrumentation
    char* test = "__AFL_SHM_ID"; // this makes AFL happy
    //webcam_trace_init();


    //FILE *logfile = fopen("/tmp/phpout2.log","a+");
//    fprintf(logfile, "---------------------------- ENV VARS2 %d ----------------------------------\n", getpid());
//
...
*/
    ...
    webcam_trace_init("__AFL_SHM_ID");

    printf("\t>>>PHP>>> cgi_main: AFTER webcam_trace_init\n");
...
parent_out:
   ...
   // FINISH OUT data gathering for AFL
    webcam_trace_finish();
    //afl_cgi_trace_finish();
	return exit_status;
}
/* }}} */
