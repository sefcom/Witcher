...
DEFAULT_CMD = ["python", "-m", "thewitcher", "/test", "EXWICHR"]
...
class WitcherTest(unittest.TestCase):
    ...
    def start_fuzzer(self, name, cmd=None, timeout=120):
    ...
    def test_fuzz_startup(self):
        ...
        self.start_fuzzer(tname, DEFAULT_CMD, timeout=5)

        with open('/tmp/unitest_out_popen.log', "rb") as fp:
            results = fp.read()

        self.assertTrue(results.find(b"fuzzers running until first crash or timeout") > -1, "Fuzzers are not running")
    ...
    def test_afuzz_znext(self):
        ...
        self.start_fuzzer(tname, DEFAULT_CMD, timeout=45)
    ...
    def test_afuzz_sub_indexing_and_skip(self):
        """
        ...
        self.start_fuzzer(tname, DEFAULT_CMD, timeout=200)
